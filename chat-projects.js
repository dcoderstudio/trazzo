(function(root){
  ...
  function empty(v){return !v||['-','sin asignar','sin cliente','sin fecha','sin monto','sin categoria','sin categoría'].includes(norm(v));}
  ...
  function parse(text,data){
    ...
    text.split(/\r?\n/).forEach(function(raw,i){
      ...
      if(parts.length===6&&norm(parts[0])==='nombre'&&norm(parts[2]).indexOf('categoria')===0)return; // fila de encabezado
      try{
        if(parts.length!==6)throw Error('Línea '+(i+1)+': usa Nombre | Cliente | Categoría | Fecha límite | Monto | Responsable.');
        var name=parts[0];
        ...
        var deadline=empty(parts[3])?null:parts[3];
        ...
        var monto=empty(parts[4])?'':parts[4];
        var member=resolve(parts[5],members,'el responsable',i+1,function(m){return m.name;});
        items.push({name:name,client:client||'—',tipoCatId:cat.id,tipoCatLabel:cat.label,deadline:deadline||'',monto:monto,resp:member?member.id:null,respName:member?member.name:'Sin asignar'});
  ...
  function open(options){
    ...
    el('p','Una solicitud por línea: Nombre | Cliente | Categoría | Fecha límite | Monto | Responsable. Fecha: AAAA-MM-DD. Usa - para dejar un campo sin asignar (Categoría es obligatoria). No uses | dentro del texto.',guide);
    var names=el('p','',guide);
    var cats=(options.categories||[]).filter(function(c){return c.id!=='cotizaciones';});
    names.textContent='Categorías: '+cats.map(function(c){return c.label;}).join(', ')+'. Responsables: '+(options.teamMembers||[]).map(function(m){return m.name;}).join(', ')+'.';
    ...
    input.placeholder='Nombre | Cliente | Categoría | Fecha límite | Monto | Responsable\nPorta papeles Novo | Novo Nordisk | Branding | 2026-10-15 | 12000 | -';
    ...
    review.onclick=function(){
      ...
      try{
        var data={categories:options.categories||[],teamMembers:options.teamMembers||[]};
        var result=parse(input.value,data);
        if(result.errors.length)throw Error(result.errors.join('\n'));
        var table=el('table','',preview),head=el('tr','',el('thead','',table));
        ['Nombre','Cliente','Categoría','Fecha','Monto','Responsable'].forEach(function(v){el('th',v,head);});
        var body=el('tbody','',table);
        result.items.forEach(function(t){var row=el('tr','',body);[t.name,t.client,t.tipoCatLabel,t.deadline||'Sin fecha',t.monto||'Sin monto',t.respName].forEach(function(v){el('td',v,row);});});
    ...
    create.onclick=function(){
      ...
      try{
        var data={categories:options.categories||[],teamMembers:options.teamMembers||[]};
        var result=parse(reviewed,data);
        if(result.errors.length)throw Error(result.errors.join('\n'));
